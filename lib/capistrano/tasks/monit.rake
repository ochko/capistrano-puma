  ...
  namespace :monit do
    ...
    task :monitor do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "#{fetch(:puma_monit_bin)} -c #{fetch(:puma_monit_rc)} monitor #{puma_monit_service_name}"
      end
    end
    ...
    task :unmonitor do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "#{fetch(:puma_monit_bin)} -c #{fetch(:puma_monit_rc)} unmonitor #{puma_monit_service_name}"
      end
    end
    ...
    task :start do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "#{fetch(:puma_monit_bin)} -c #{fetch(:puma_monit_rc)} start #{puma_monit_service_name}"
      end
    end
    ...
    task :stop do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "#{fetch(:puma_monit_bin)} -c #{fetch(:puma_monit_rc)} stop #{puma_monit_service_name}"
      end
    end
    ...
    task :restart do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "#{fetch(:puma_monit_bin)} -c #{fetch(:puma_monit_rc)} restart #{puma_monit_service_name}"
      end
    end
